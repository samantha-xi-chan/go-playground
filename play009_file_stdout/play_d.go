...
func getLogFileV2(taskId string) string {
	return "./log.txt"
}
...
func PlayD() {
	...
	file, err := os.Create(getLogFileV2(""))
...
func trackV2(chanSig chan int, taskId string) (exit int) {
	chanText := make(chan string)
	t, err := tail.TailFile(getLogFileV2(taskId), tail.Config{Follow: true, ReOpen: true})
	if err != nil {
		return 0
	}
